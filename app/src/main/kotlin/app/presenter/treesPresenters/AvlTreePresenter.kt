...
import bstrees.model.dataBases.serialize.strategies.AvlStrategy
...
object AvlTreePresenter {
    fun addNode(tree: SerializableTree, key: String, value: String): SerializableTree{
        return when(tree.keyType to tree.valueType){
            "String" to "String" -> {
                val strategy = AvlStrategy(::stringToString, ::stringToString, ::stringToString, ::stringToString, tree.keyType, tree.valueType)
                val bTree = strategy.deserializeTree(tree)
                bTree.insert(key, value)
                strategy.serializeTree(bTree, tree.name)
            }
            "Int" to "Int" -> {
                val strategy = AvlStrategy(::intToString, ::intToString, ::stringToInt, ::stringToInt, tree.keyType, tree.valueType)
                val bTree = strategy.deserializeTree(tree)
                bTree.insert(key.toInt(), value.toInt())
                strategy.serializeTree(bTree, tree.name)
            }
            "String" to "Int" -> {
                val strategy = AvlStrategy(::stringToString, ::intToString, ::stringToString, ::stringToInt, tree.keyType, tree.valueType)
                val bTree = strategy.deserializeTree(tree)
                bTree.insert(key, value.toInt())
                strategy.serializeTree(bTree, tree.name)
            }
            "Int" to "String" -> {
                val strategy = AvlStrategy(::intToString, ::stringToString, ::stringToInt, ::stringToString, tree.keyType, tree.valueType)
                val bTree = strategy.deserializeTree(tree)
